...
# 初始化 YOLOv8 模型
model = YOLO("E:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\best.pt")

# 處理整個資料夾中的圖像
folder_path = "E:\\Lab\\share\\dataset\\two_label_data_forCNN_v24\\PA_images"
for filename in os.listdir(folder_path):
    if filename.endswith(".jpg"):
        ...
        for i, idx in enumerate(sorted_indices):
            ...
            # 保存旋转后的图像
            output_folder_rotated = 'E:\\Lab\\share\\dataset\\two_label_data_forCNN_v24\\PA_rotated_images'
            ...
            # 保存边界框信息
            output_folder_bbox = 'E:\\Lab\\share\\dataset\\two_label_data_forCNN_v24\\PA_bbox_info'
            ...
            # 保存提取的图像部分
            output_folder = 'E:\\Lab\\share\\dataset\\two_label_data_forCNN_v24\\cropped_images'
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            cropped_image.save(os.path.join(output_folder, f'{original_filename}_{i+1}.jpg'))

        # 保存预测结果
        output_folder = 'E:\\Lab\\share\\dataset\\two_label_data_forCNN_v24\\PA_predicts'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        for r in results:
