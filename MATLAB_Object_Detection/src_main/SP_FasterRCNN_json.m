...
% 1. 将 T_bbox 列转换为 cell 数组，确保每个元素都是 M-by-4、M-by-5 或 M-by-9 的矩阵
gTruth_labeler_SN.T_bbox = cellfun(@(x) {x}, gTruth_labeler_SN.T_bbox);
gTruth_labeler2_SN.T_bbox2 = cellfun(@(x) {x}, gTruth_labeler2_SN.T_bbox2);
...
imdsTrain = imageDatastore(gTruth_labeler_SN.TrainImg);
bldsTrain = boxLabelDatastore(gTruth_labeler_SN(:,2:end));

% Test
imdsTest = imageDatastore(gTruth_labeler2_SN.TestImg);
bldsTest = boxLabelDatastore(gTruth_labeler2_SN(:,2:end));
