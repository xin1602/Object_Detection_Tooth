...
# 初始化 YOLOv8 模型
model = YOLO("F:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\best.pt")
...
for filename in os.listdir(folder_path):
    if filename.endswith(".jpg"):
        ...
        for i, idx in enumerate(sorted_indices):
            ...
            # 保存旋转后的图像
            output_folder_rotated = 'F:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\rotated_images'
            if not os.path.exists(output_folder_rotated):
                os.makedirs(output_folder_rotated)
            rotated_image_path = os.path.join(output_folder_rotated, f'{os.path.splitext(os.path.basename(image_path))[0]}_{i+1}_rotated.jpg')
            ...
            # 保存边界框信息
            output_folder_bbox = 'F:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\bbox_info'
            ...
            # 保存提取的图像部分
            output_folder = 'F:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\cropped_images'
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            cropped_image.save(os.path.join(output_folder, f'{original_filename}_{i+1}.jpg'))

        # 保存预测结果
        output_folder = 'F:\\Lab\\share\\YOLO_segmentation\\seg_0325_tooth_data_v1_0308_GRAY_AHE_01\\predicts'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        for r in results:
