% 設定原始圖片資料夾
original_folder = 'D:\lxh\dataset\train_single\original_images\apical lesion';
% original_folder = 'D:\lxh\dataset\test_single\original_images\apical lesion';

% 設定增強後圖片保存資料夾
enhanced_folder = 'D:\lxh\dataset\train_single\enhancement_images\apical lesion';
...
for i = 1:length(image_files)
    ...
    imwrite(flipped_horizontal, fullfile(enhanced_folder, strcat('flipped_horizontal_', image_files(i).name)));
    
    % 資料增強：垂直翻轉
    flipped_vertical = flip(original_image, 1);
    imwrite(flipped_vertical, fullfile(enhanced_folder, strcat('flipped_vertical_', image_files(i).name)));
    
    % 資料增強：水平和垂直翻轉
    flipped_both = flip(flipped_horizontal, 1);
    imwrite(flipped_both, fullfile(enhanced_folder, strcat('flipped_both_', image_files(i).name)));
